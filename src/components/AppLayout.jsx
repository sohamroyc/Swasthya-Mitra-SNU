...
const AppLayout = ({ children, activeTab }) => {
    ...
    const planRole = plan === 'family' ? 'Family Plan' : plan === 'plus' ? 'Plus Member' : 'Free Member';
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                setProfile({ name: user?.name || data.name, role: planRole });
            } catch (err) {
    ...
    const navLinks = [
        ...
        { id: 'subscription', path: '/subscription', label: 'Plans & Billing', iconComponent: CreditCard },
    ];
        ...
        <div className="min-h-screen bg-[#F8FAFF] dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-display flex flex-col">
            ...
            <header className="sticky top-0 z-50 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 px-4 md:px-6 py-3 flex items-center justify-between shadow-sm select-none">
                ...
                <div className="flex items-center gap-2 md:gap-3 shrink-0">
                    ...
                    {/* Plan Badge / Upgrade CTA - Desktop only */}
                    <Link
                        to={plan === 'free' ? '/pricing' : '/subscription'}
                        className={`hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all shadow-xs ${
                            plan === 'free'
                                ? 'bg-blue-50 hover:bg-blue-100 dark:bg-blue-950/40 text-[#1A6FE8] dark:text-[#4F92F8] border border-blue-200/60 dark:border-blue-800/60'
                                : plan === 'family'
                                ...
                                : 'bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200/60 dark:border-emerald-800/60'
                        }`}
                    >
                        <Sparkles className="w-3 h-3" />
                        <span>{plan === 'free' ? 'Upgrade to Plus' : plan === 'family' ? 'Family Plan' : 'Plus Active'}</span>
                    </Link>
