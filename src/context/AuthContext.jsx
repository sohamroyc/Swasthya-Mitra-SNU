...
export const AuthProvider = ({ children }) => {
                ...
                try {
                    ...
                    if (session?.user) {
                        ...
                        if (!error && data) {
                            ...
                            const googleAvatar = meta.avatar_url || meta.picture || `https://ui-avatars.com/api/?name=${encodeURIComponent(googleName)}&background=0f6df0&color=fff`;
                            const newProfile = {
                            ...
                            try {
                                ...
                                console.warn("Auto-upsert profile on session init error:", e);
                            }
                            const mappedUser = {
                                name: googleName,
                                email: email.toLowerCase(),
                    ...
                    try {
                        ...
                        if (!error && data) {
                            ...
                            try {
                                ...
                                console.warn("Auto-upsert profile on auth change error:", e);
                            }
                            const mappedUser = {
                                name: googleName,
                                email: email.toLowerCase(),
    ...
    const loginWithGoogle = async (accountOverride = null) => {
        if (accountOverride) {
            const chosenName = accountOverride.name || 'Google User';
            const chosenEmail = (accountOverride.email || 'googleuser@gmail.com').toLowerCase();
            const chosenAvatar = accountOverride.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(chosenName)}&background=0f6df0&color=fff`;

            const googleUser = {
                name: chosenName,
                email: chosenEmail,
                phone: accountOverride.phone || '',
                dob: accountOverride.dob || '1995-05-15',
                gender: accountOverride.gender || 'prefer-not-to-say',
                bloodType: accountOverride.bloodType || 'O+',
                height: 175,
                weight: 70,
                allergies: 'None',
                conditions: 'None',
            ...
            // If Supabase is available, sync to profiles table
            if (isSupabaseConfigured()) {
                try {
                    await supabase.from('profiles').upsert([{
                        email: chosenEmail,
                        name: chosenName,
                        photo_url: chosenAvatar,
                        created_at: googleUser.createdAt
                    }], { onConflict: 'email' });
                } catch (err) {
                    console.warn("Supabase upsert for Google user failed:", err);
                }
            }
