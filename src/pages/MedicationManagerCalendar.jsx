...
import { Bell, BellRing, Sparkles, Users, Lock } from 'lucide-react';

const MedicationManagerCalendar = () => {
    const { user } = useAuth();
    const { hasAccess, plan, familyMembers } = useSubscription();
    ...
    return (
        <AppLayout activeTab="medications">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
                        <div>
                            <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {hasAccess('medication_reminders') ? (
                                    ...
                                    <button
                                        ...
                                        className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-slate-100 hover:bg-blue-50 text-slate-600 hover:text-blue-600 text-[10px] font-bold border border-slate-200 transition-colors"
                                    >
                            ...
                            <p className="text-slate-500 font-medium mt-1 text-base">Adherence score: <span className="text-emerald-600 font-bold">95%</span> this month</p>
                        </div>
                        <div className="flex items-center gap-2 flex-wrap">
                            {hasAccess('shared_medications') && familyMembers?.length > 0 && (
                                <select
                                    value={selectedFamilyMember}
                                    onChange={(e) => setSelectedFamilyMember(e.target.value)}
                                    className="px-3 py-2 rounded-xl text-xs font-bold border border-indigo-200 bg-indigo-50 text-indigo-900 focus:outline-none"
                                >
                                    <option value="all">All Household Profiles</option>
                                    <option value="self">Primary Account (Me)</option>
                                    {familyMembers.map((m) => (
                                        <option key={m.id} value={m.id}>
                                            {m.member_name} ({m.relationship})
                                        </option>
                                    ))}
                                </select>
                            )}
                            <button onClick={() => setIsModalOpen(true)} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 shrink-0 text-xs">
            ...
            {/* Upgrade Modal for Reminders */}
            <UpgradeModal
                isOpen={isUpgradeModalOpen}
                onClose={() => setIsUpgradeModalOpen(false)}
                featureName="Medication Alerts & Reminders"
            />
        </AppLayout>
    );
};
